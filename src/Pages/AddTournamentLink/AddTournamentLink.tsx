...
function AddTournamentLink() {
  ...
  const { isLoading, error, errorsFilling, handleParseLink, isSuccessParse, isSuccessSave, handleSaveTournament } =
    useSaveTournament();
  ...
  return (
    <div className={styles.container}>
      <div className={styles.addlink}>
        ...
        />
        <Button title='Открыть' onClick={() => handleParseLink(dispatch, link)} disabled={isLoading} />
      </div>
      ...
      {error && <p className={styles.errorMessage}>{getServerErrorMessage(error, 'Ошибка сервера')}</p>}
