...
export const reducer = (state: TournamentType, action: Action) => {
	...
	switch (type) {
		...
		case actionTypes.removeSource: {
			...
			const questionIndexRemove = removeSourceQuestions.findIndex(
				(q) => q.id === action.questionID,
			);

			if (removeSourceQuestions[questionIndexRemove].source) {
				const sourcesRemove = removeSourceQuestions[questionIndexRemove].source.filter(
					(s) => s.id !== action.sourceID,
				);
